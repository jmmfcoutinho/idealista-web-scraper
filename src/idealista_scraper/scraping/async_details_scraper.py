"""Async details scraper for enriching listings with full details.

This module implements an async version of the details scraper that visits
individual listing pages concurrently to enrich the database with additional
information like descriptions, energy ratings, features, and characteristics.
"""
...
from idealista_scraper.scraping.client import WAIT_SELECTOR_LISTING_DETAIL
...
class DetailFetchResult:
    """Result of fetching a single listing detail page.

    Attributes:
        listing_id: The database listing ID.
        idealista_id: The Idealista listing ID.
        detail: Parsed listing detail if successful, None otherwise.
        error: Error message if failed, None otherwise.
    """

    listing_id: int
    idealista_id: int
    detail: ParsedListingDetail | None
    error: str | None = None


class AsyncDetailsScraper:
    """Async scraper for enriching listings with detail page data.

    Uses asyncio.gather() with Semaphore to fetch multiple detail pages
    concurrently while respecting concurrency limits.
    ...
    def __init__(
        ...
        """Initialize the AsyncDetailsScraper.
    ...
    async def run(self) -> dict[str, int]:
        """Scrape details for a subset of listings concurrently.
        ...
        """
        logger.info(
            "Starting async details scraper (max_listings=%s, concurrency=%d)",
            self.max_listings,
            self.concurrency,
        )
        ...
        try:
            ...
            # Process listings in batches for concurrent fetching
            ...
            for batch_start in range(0, total_to_process, batch_size):
                ...
                logger.info(
                    "Processing batch %d-%d of %d listings",
                    batch_start + 1,
                    batch_end,
                    total_to_process,
                )

                # Fetch all details in batch concurrently
                results = await self._fetch_details_batch(batch)

                # Process results sequentially for DB updates
                for result in results:
                    ...
                    if result.detail:
                        # Find the listing in our batch
                        listing = next(
                            (li for li in batch if li.id == result.listing_id), None
                        )
                        if listing:
                            self._update_listing_from_detail(listing, result.detail)
                            stats["listings_enriched"] += 1
                            logger.debug(
                                "Enriched listing %d: description=%s, energy=%s",
                                result.idealista_id,
                                "yes" if result.detail.description else "no",
                                result.detail.energy_class or "none",
                            )
            ...
            logger.info(
                "Async details scraper completed: %d processed, %d enriched, %d failed",
            ...
            logger.exception("Async details scraper failed: %s", e)
    ...
    async def _fetch_detail(self, listing: Listing) -> DetailFetchResult:
        """Fetch detail page for a single listing with semaphore control.

        Args:
            listing: The listing to fetch details for.

        Returns:
            DetailFetchResult with parsed detail or error.
        """
    ...
    async def _fetch_details_batch(
        ...
        """Fetch detail pages for a batch of listings concurrently.

        Args:
            listings: List of listings to fetch details for.

        Returns:
            List of DetailFetchResult objects.
        """
        tasks = [self._fetch_detail(listing) for listing in listings]
